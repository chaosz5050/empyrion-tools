...
class PlayerDatabase:
    ...
    def get_entities(self):
        """
        Get all entities from the database with last refresh time.
        """
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                ...
                # Get entities
                cursor.execute("SELECT * FROM entities ORDER BY playfield, type, name")
                entities = [dict(row) for row in cursor.fetchall()]
                
                # Get last refresh time
                cursor.execute("SELECT last_refresh FROM entities_meta WHERE id = 1")
                row = cursor.fetchone()
                ...
                stats = {
                    'total': len(entities),
                    'asteroids': len([e for e in entities if 'astvoxel' in e.get('type', '').lower() or 'asteroid' in e.get('type', '').lower()]),
                    'structures': len([e for e in entities if e.get('type', '') == 'BA']),
                    'ships': len([e for e in entities if e.get('type', '') in ['CV', 'SV', 'HV']]),
                    'wrecks': len([e for e in entities if 'wreck' in e.get('name', '').lower()])
                }
            ...
            return {
                ...
                'entities': [],
                'stats': {'total': 0, 'asteroids': 0, 'structures': 0, 'ships': 0, 'wrecks': 0}
            }

    def save_entities(self, entities, raw_data=None):
        """
        Save entities to the database and update last refresh time.
        """
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                ...
                for entity in entities:
                    cursor.execute("""
                        INSERT INTO entities (id, name, type, faction, playfield, time_info)
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, (
                        entity.get('id', ''),
                        entity.get('name', ''),
                ...
                cursor.execute("""
                    INSERT OR REPLACE INTO entities_meta (id, last_refresh, raw_data)
                    VALUES (1, ?, ?)
                """, (refresh_time, raw_data or ''))
    ...
    def purge_old_players(self, days_threshold=14, dry_run=False):
        """
        Purge old player data and duplicates from the database.
        
        Removes:
        - Players with NULL last_seen data
        - Players who haven't been seen in the last X days
        - Duplicate entries (keeps newest first_seen)
        
        Args:
            days_threshold (int): Days since last_seen to consider old (default: 14)
            dry_run (bool): If True, only count without deleting
            
        Returns:
            dict: {'success': bool, 'message': str, 'old_count': int, 'duplicate_count': int, 'total_deleted': int}
        """
        try:
            # Validate threshold
            if days_threshold < 1:
                return {'success': False, 'message': 'Days threshold must be at least 1'}
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Count total players first (safety check)
                cursor.execute("SELECT COUNT(*) FROM players")
                total_players = cursor.fetchone()[0]
                
                # Find old players (NULL last_seen OR last_seen < cutoff_date)
                cursor.execute("""
                    SELECT steam_id, name, last_seen FROM players 
                    WHERE last_seen IS NULL OR last_seen < ?
                """, (cutoff_date,))
                old_players = cursor.fetchall()
                old_count = len(old_players)
                
                # Find duplicates by name (keep most recent first_seen)
                cursor.execute("""
                    SELECT p1.steam_id, p1.name, p1.first_seen
                    FROM players p1
                    WHERE EXISTS (
                ...
                total_to_delete = old_count + duplicate_count
                
                # Safety check: Never delete all players
                remaining_players = total_players - total_to_delete
                if remaining_players < 5:
                    return {
                        'success': False, 
                        'message': f'Safety check failed: Would leave only {remaining_players} players. Minimum is 5.',
                        'old_count': old_count,
                ...
                logger.info(f"Purge analysis: {old_count} old players, {duplicate_count} duplicates")
                for steam_id, name, last_seen in old_players:
                    logger.info(f"Old player: {name} ({steam_id}) - Last seen: {last_seen}")
                for steam_id, name, first_seen in duplicate_players:
                    logger.info(f"Duplicate player: {name} ({steam_id}) - First seen: {first_seen}")
                
                if dry_run:
                    return {
                        'success': True,
                        'message': f'Dry run: Would delete {total_to_delete} players ({old_count} old, {duplicate_count} duplicates)',
                        'old_count': old_count,
                        'duplicate_count': duplicate_count,
                        'total_deleted': 0
                    }
                
                # Actually delete (in transaction)
                deleted_count = 0
                
                # Delete old players
                if old_players:
                    old_steam_ids = [p[0] for p in old_players]
                    placeholders = ','.join('?' * len(old_steam_ids))
                    cursor.execute(f"DELETE FROM players WHERE steam_id IN ({placeholders})", old_steam_ids)
                    deleted_count += cursor.rowcount
                
                # Delete duplicate players
                if duplicate_players:
                    dup_steam_ids = [p[0] for p in duplicate_players]
                    placeholders = ','.join('?' * len(dup_steam_ids))
                    cursor.execute(f"DELETE FROM players WHERE steam_id IN ({placeholders})", dup_steam_ids)
                    deleted_count += cursor.rowcount
                
                conn.commit()
                logger.info(f"Player purge completed: {deleted_count} players removed")
                
                return {
                    'success': True,
                    'message': f'Successfully deleted {deleted_count} players ({old_count} old, {duplicate_count} duplicates)',
                    'old_count': old_count,
                    'duplicate_count': duplicate_count,
            ...
            return {
                'success': False,
                'message': f'Database error during purge: {e}',
                'old_count': 0,
                'duplicate_count': 0,
                'total_deleted': 0
            }
    ...
    def backup_database(self):
        ...
        Creates players.db.bak in the same directory.
    ...
    def restore_database(self):
        ...
        Restores from players.db.bak to players.db.
        ...
        try:
            ...
            if not os.path.exists(backup_path):
                return {
                    'success': False,
                    'message': 'No backup file found. Create a backup first.'
                }
    ...
    def export_players_csv(self):
        ...
        Returns:
            dict: {'success': bool, 'message': str, 'csv_data': str, 'player_count': int}
        """
        try:
            import csv
            import io
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Write header
                writer.writerow([
                    'Steam ID', 'Name', 'Status', 'Faction', 'Role', 'IP Address', 
                    'Country', 'Playfield', 'Last Seen', 'First Seen', 'Total Playtime', 'Updated At'
                ])
                
                # Write player data
                for player in players:
                    writer.writerow([
                        player['steam_id'], player['name'], player['status'], 
                        player['faction'], player['role'], player['ip_address'],
                        player['country'], player['playfield'], player['last_seen'],
                        player['first_seen'], player['total_playtime'], player['updated_at']
                    ])
                
                csv_data = output.getvalue()
                output.close()
                
                logger.info(f"Exported {len(players)} players to CSV")
                return {
                    'success': True,
                    'message': f'Successfully exported {len(players)} players',
                    'csv_data': csv_data,
