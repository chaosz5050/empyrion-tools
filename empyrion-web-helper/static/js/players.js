...
window.PlayersManager = {
    ...
    async purgeOldPlayers() {
        try {
            ...
            const previewData = await apiCall('/players/purge?dry_run=true', { method: 'GET' });
            
            if (!previewData.success) {
                showToast(previewData.message || 'Failed to preview purge operation', 'error');
                return;
            }
            
            const oldCount = previewData.old_count || 0;
            const duplicateCount = previewData.duplicate_count || 0;
            const totalCount = oldCount + duplicateCount;
            
            if (totalCount === 0) {
                showToast('No old player data or duplicates found to purge', 'info');
                return;
            }
            
            // Show detailed confirmation dialog
            let confirmMessage = `Found ${totalCount} player records to delete:\n`;
            if (oldCount > 0) {
                confirmMessage += `• ${oldCount} old players (>14 days inactive or missing activity)\n`;
            }
            if (duplicateCount > 0) {
                confirmMessage += `• ${duplicateCount} duplicate entries\n`;
            }
            ...
            if (data.success) {
                ...
                if (deletedCount > 0) {
                    showToast(`Successfully deleted ${deletedCount} player records`, 'success');
                    // Refresh the player list to reflect changes
                    setTimeout(() => this.loadPlayersFromDatabase(), 1000);
                } else {
                    showToast('No players were deleted', 'info');
                }
            } else {
    ...
    async backupDatabase() {
        try {
            const data = await apiCall('/players/backup', { method: 'POST' });
            
            if (data.success) {
                showToast('Database backup created successfully', 'success');
            } else {
                showToast(data.message || 'Failed to create database backup', 'error');
            }
        } catch (error) {
            console.error('Error creating database backup:', error);
            showToast('Error creating database backup: ' + error, 'error');
        }
    },

    async restoreDatabase() {
        const confirmMessage = 'Are you sure you want to restore the database from backup?\n\nThis will overwrite all current player data with the backup data.\nThis action cannot be undone.';
        ...
        try {
            const data = await apiCall('/players/restore', { method: 'POST' });
            
            if (data.success) {
                showToast('Database restored successfully from backup', 'success');
                // Refresh the player list to reflect restored data
                setTimeout(() => this.loadPlayersFromDatabase(), 1000);
            } else {
            ...
            showToast('Error restoring database: ' + error, 'error');
        }
    },
    ...
    async exportPlayersCSV() {
        try {
            ...
            window.open('/players/export-csv', '_blank');
            showToast('Player data export started', 'info');
        } catch (error) {
            console.error('Error exporting players CSV:', error);
            showToast('Error exporting players CSV: ' + error, 'error');
        }
    },

    // No longer needed - manual refresh button removed
    enablePlayersFeatures(enabled) {
        // Background service handles all player updates automatically
    }
};
...
function backupDatabase() {
    window.PlayersManager.backupDatabase();
}

function restoreDatabase() {
    window.PlayersManager.restoreDatabase();
}

function exportPlayersCSV() {
    window.PlayersManager.exportPlayersCSV();
}
